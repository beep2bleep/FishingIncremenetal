...
func configure(target_root: Node, layer_index: int = 2) -> CrtTextMirrorOverlay:
...
func _collect_text_controls(node: Node) -> void:
    for child in node.get_children():
        if child == self:
            continue
        if child.name in ["EditorCrtOverlay", "EditorCrtTextMirrorOverlay", "MiningCrtOverlay", "MiningTextMirrorOverlay"]:
            continue
        if child is Label:
