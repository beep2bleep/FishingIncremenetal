...
func _run_all() -> void:
	...
	_write_overview(summary_rows)
	print("20-run passes complete.")
	quit()
...
func _run_single_pass(pass_name: String, seed_value: int) -> Dictionary:
	...
	for run_index in range(1, RUNS_PER_PASS + 1):
	...
	var avg_upgrade_count: float = 0.0
	var avg_kill_delta: float = 0.0
	for run_variant in runs:
		var run: Dictionary = run_variant
		avg_upgrade_count += float(run.get("upgrades_bought_count", 0))
		avg_kill_delta += float(run.get("kill_delta", 0))
	avg_upgrade_count /= max(1.0, float(runs.size()))
	avg_kill_delta /= max(1.0, float(runs.size() - 1))
	...
	return {
		...
		"runs": runs,
		"avg_upgrades_per_run": avg_upgrade_count,
...
func _write_pass_reports(pass_name: String, result: Dictionary) -> void:
	var abs_base: String = ProjectSettings.globalize_path(OUTPUT_DIR) + "/" + pass_name
	...
	md.append("# 20-Run BattleScene Infinite Report - %s" % pass_name)
	md.append("")
	...
	md.append("- Final currency: `%d`" % int(result.get("final_currency", 0)))
	md.append("- Final level: `%d`" % int(result.get("final_level", 1)))
	md.append("")
...
func _write_overview(rows: Array[Dictionary]) -> void:
	var md: PackedStringArray = []
	md.append("# 20-Run Simulation Overview")
	md.append("")
	md.append("| Pass | Seed | Avg Upgrades/Run | Avg Kill Delta/Run | Final Currency | Final Level |")
	md.append("|---|---:|---:|---:|---:|---:|")
	for row_variant in rows:
		var row: Dictionary = row_variant
		md.append("| %s | %d | %.2f | %.2f | %d | %d |" % [
			str(row.get("pass", "")),
			int(row.get("seed", 0)),
			float(row.get("avg_upgrades_per_run", 0.0)),
			float(row.get("avg_kill_delta_per_run", 0.0)),
			int(row.get("final_currency", 0)),
			int(row.get("final_level", 1)),
		])

	var path: String = ProjectSettings.globalize_path(OUTPUT_DIR) + "/overview.md"
