...
const DEFAULT_DATA := {
    ...
    "oil_upgrades": {},
    "destroyed_cells": [],
    "last_run_summary": "No Open Pit Empire run completed yet.",
    "last_run_breakdown": {},
    "boss_defeated": false,
}
...
static func load_data() -> Dictionary:
    ...
    data["oil_wallet"] = max(0, int(data.get("oil_wallet", 0)))
    if not (data.get("upgrades", {}) is Dictionary):
        data["upgrades"] = {}
    if not (data.get("oil_upgrades", {}) is Dictionary):
        data["oil_upgrades"] = {}
...
static func get_oil_wallet() -> int:
    return int(load_data().get("oil_wallet", 0))
...
static func get_upgrade_level(upgrade_id: String) -> int:
    return int(load_data().get("upgrades", {}).get(upgrade_id, 0))
...
static func apply_run_results(results: Dictionary) -> Dictionary:
    ...
    data["oil_wallet"] = max(0, int(data.get("oil_wallet", 0)) + int(results.get("oil", 0)))
    data["last_run_summary"] = str(results.get("summary_text", "Open Pit run complete."))
...
static func get_depth_level_for_display_tier(display_tier: int) -> int:
    return clampi(display_tier, MIN_START_DEPTH_LEVEL, MAX_DEPTH_LEVEL)
